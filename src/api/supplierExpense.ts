...
// 寄售(expenseType=3) 现给/到期给皆可：现给=陈列费当场付清、单状态直接已结清、到期日仅作货物处置提醒；返钱默认现给、返货默认到期给
...
// 货物处置状态（独立于结算状态）：0=待处置 1=已拉走 2=已续约；仅寄售铺货(expenseType=3)使用
export const DISPOSAL_STATUS_LABEL: Record<number, string> = { 0: '待处置', 1: '已拉走', 2: '已续约' };
...
export interface SupplierExpense {
  ...
  // 货物处置状态（独立于结算状态）：0=待处置 1=已拉走 2=已续约；仅寄售铺货(expenseType=3)
...
export type ExpensePayload = {
  ...
  consignReturnItems?: ReturnItem[];
  // 货物处置状态（寄售铺货现给/到期给均带，独立于结算状态）：0=待处置 1=已拉走 2=已续约
  disposalStatus?: number;
...
// 货物处置状态独立更新（独立于结算状态）：PUT /api/supplier-expenses/:id/disposal
export async function updateDisposalStatus(baseUrl: string, id: number, disposalStatus: number): Promise<SupplierExpense> {
  return apiJson<SupplierExpense>(baseUrl, `/api/supplier-expenses/${id}/disposal`, {
    method: 'PUT',
    body: JSON.stringify({ disposalStatus }),
  });
}
